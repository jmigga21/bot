...
from design import products, profile, rules, faq, support, refill, faq_chat_inl, faq_news_inl, support_inl, ref_system, promocode, \
...
def user_menu(user_id):
    ...
    kb.append(InlineKeyboardButton(rules, callback_data="rules:open"))
    
    keyboard.add(kb[0], kb[1])
    keyboard.add(kb[2], kb[6])
...
def open_products():
    ...
    for category in get_all_categories():
        name = category['name']
        cat_id = category['id']
        keyboard.add(InlineKeyboardButton(f"💠 {name}", callback_data=f"open_category:{cat_id}"))
...
def open_pod_cat_positions(pod_cat_id):
    ...
    for pos in get_positions(pod_cat_id=pod_cat_id):
        ...
        if pos['infinity'] == "+":
            items = "[Безлимит]"
        keyboard.add(InlineKeyboardButton(f"📦 {name} | {price}₽", callback_data=f"open_pos:{pos_id}"))
...
def open_positions(cat_id):
    ...
    for pod_cat in get_pod_categories(cat_id):
        ...
        keyboard.add(InlineKeyboardButton(f"🏡 {name}", callback_data=f"open_pod_cat:{pod_cat_id}"))
...
def pos_buy_inl(pos_id):
    ...
    keyboard.add(InlineKeyboardButton("📦 Купить", callback_data=f"buy_pos:{pos_id}"))
