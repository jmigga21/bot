...
def create_db():
    with sqlite3.connect(path_db) as con:
        ...
        # Настройки
        if len(con.execute("PRAGMA table_info(settings)").fetchall()) == 19:
            ...
            con.execute("INSERT INTO settings("
                        ...
                        "profit_day, profit_week, rules)"
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        ["True", "False", "False", "False", "True", "False", "None", 0, 0, 0, 0, 0, 0, "None", get_unix(), get_unix(), "None"])
