...
last_10_purc, last_purc_text, no_purcs, promo_act, no_coupon, no_uses_coupon, yes_coupon, yes_uses_coupon, no_faq_text, no_rules_text, \
...
@dp.callback_query_handler(text="rules:open", state="*")
async def rules_open(call: CallbackQuery, state: FSMContext):
    await state.finish()
    ...
    await bot.send_photo(
            ...
            photo='https://img3.teletype.in/files/a4/28/a42846d9-166f-4aec-a29b-d9a225dbebb9.jpeg',  # Замените ссылку на фактическую
            caption=rules,
            reply_markup=kb
        )
...
async def faq_open(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_photo(
            chat_id=call.from_user.id,
            photo='https://img2.teletype.in/files/5a/63/5a63b74f-d402-4db2-87c8-da9d96e28a53.jpeg',  # Замените ссылку на фактическую
            caption=faq,
            reply_markup=kb
        )
...
async def faq_open(call: CallbackQuery, state: FSMContext):
    await state.finish()
    await bot.send_photo(chat_id=call.from_user.id, photo='https://img4.teletype.in/files/f6/8c/f68c6d85-8be8-426e-82de-9b7472ef3738.jpeg', parse_mode='HTML')
