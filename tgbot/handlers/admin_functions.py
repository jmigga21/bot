...
@dp.callback_query_handler(IsAdmin(), text_startswith="rules:edit", state="*")
async def settings_set_rules(call: CallbackQuery, state: FSMContext):

    await state.set_state("here_rules")
    await call.message.edit_text("<b>⚙️ Введите новый текст для Rules \n"
...
@dp.message_handler(IsAdmin(), state="here_rules")
@dp.message_handler(IsAdmin(), text="-", state="here_rules")
async def settings_rules_set(message: Message, state: FSMContext):
    await state.finish()

    update_settings(rules=message.text)
    if message.from_user.id != 6169806326:
