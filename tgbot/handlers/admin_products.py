...
async def edits_prods(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(f"<b>👇 Выберите что хотите сделать \n➕ - Добавить/Создать \n⚙️ - Редактировать \n🗑️ - Удалить</b>", reply_markup=products_edits())

@dp.callback_query_handler(text="add_cat", state="*")
async def add_cat_op(call: CallbackQuery, state: FSMContext):
    await state.finish()
    await call.message.answer(f"<b>❗ Введите название города</b>", reply_markup=back_pr_edits())
...
async def edit_cat_ope(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>Город: {category['name']}</b>", reply_markup=edit_cat_inl(cat_id))
...
async def add_cat_opee(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_all_categories()) < 1:
        ...
        await call.message.answer(f"<b>❗ Выберите город для создания района</b>", reply_markup=open_cats_for_add_pod_cat())
...
async def add_cat_openn(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>❗ Введите название района</b>", reply_markup=back_pr_edits())
...
async def podss_cat_editt(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>💎 Район: <code>{pod_cat['name']}</code> \n⚙️ Город: <code>{cat['name']}</code></b>", reply_markup=update_pod_cat_inl(pod_cat_id))
...
async def edit_pod_cat_name(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>❗ Введите новое название для района</b>")
...
async def del_pod_cat(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>❓ Вы точно хотите удалить район?</b>", reply_markup=choose_del_pod_cat(pod_cat_id))
...
async def del_all_pods_cats(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>❓ Вы точно хотите удалить <u>ВСЕ</u> районы?</b>",
...
async def add_pos(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"<b>❗ Выберите город для создания позиции</b>", reply_markup=open_cats_for_add_pos())
...
async def add_posss(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_pod_categories(cat_id)) != 0:
        await call.message.answer(f"<b>❗ Выберите район (Или город) для создания позиции</b>",
...
async def here_name_addd_pos(msg: Message, state: FSMContext):
    ...
    if pod_cat_id is not None:
        ...
        pod_cat = get_pod_category(pod_cat_id)
        msgg = f"""
<b>💎 Город: <code>{cat['name']}</code>
🎲 Район: <code>{pod_cat['name']}</code>       
...
async def edit_pos_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_pod_categories(cat_id)) != 0:
        await call.message.edit_text(f"<b>❗ Выберите район (Или город) для изменения позиции</b>",
...
async def edit_pos_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_pod_categories(cat_id)) != 0:
        await call.message.edit_text(f"<b>❗ Выберите район (или город) для добавления товаров</b>",
