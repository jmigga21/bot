...
async def open_products_users(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_all_categories()) < 1:
        ...
        await bot.send_photo(
            ...
            photo='https://img1.teletype.in/files/00/ba/00ba5b6d-d827-4bfa-ae07-34209af7d40c.jpeg',  # Замените ссылку на фактическую
            caption=available_cats,
            reply_markup=open_products(),
        )
...
async def open_cat_for_buy(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions(cat_id)) < 1:
        ...
        await bot.send_photo(
            chat_id=call.from_user.id,
            photo='https://img1.teletype.in/files/00/ba/00ba5b6d-d827-4bfa-ae07-34209af7d40c.jpeg',  # Замените ссылку на фактическую
            caption=current_cat.format(name=get_category(cat_id)['name']),
            reply_markup=open_positions(cat_id),
        )
